...
from app_paths import LOADOUT_DIR, resource_path
...
def load_errors(path=None):
    try:
        path = resource_path("err.json") if path is None else path
...
class EquipmentBotSession:
    ...
    async def connect(self, config):
        ...
        # Keep the password only in this coroutine long enough to authenticate.
        # It is never stored on the session object and is discarded after login.
        username = str(config["username"])
        password = str(config["password"])
        game_url = str(config["game_url"])
        game_server = str(config["game_server"])
        ...
        self.zone = game_server
        self.state = GameState()

        try:
            self._connect_context = websockets.connect(f"wss://{game_url}/")
